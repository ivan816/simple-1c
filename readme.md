...
информационной базы или даже на нескольких информационных база, если
используется более одного сервера СУБД. Это становится особенно актуально при
...
Используем [https://irony.codeplex.com](Irony) для синтаксического анализа и построения AST.
...
в отдельную [https://www.postgresql.org/docs/9.1/static/ddl-schemas.html](схему) simple1c в sql базе данных
...
* Поддерживается большая часть конструкций языка - все стандартные элементы sql (union, group by, order by и т.п.),
